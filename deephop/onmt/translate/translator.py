...
from preprocess import parrel_func
...
class Translator(object):
    ...
    def translate(self,
        ...
        Args:
            ...
            condition_path (str): filepath of 生成条件
        Returns:
            (`list`, `list`)
        ...
        if prepare_pt_file is None:
            pt_file = os.path.join(os.path.dirname(src_path), 'emb.pt')
        else:
            pt_file = prepare_pt_file
        if os.path.isfile(pt_file):
            ...
            data.examples = data_saved.examples
            data.src_vocabs = data_saved.src_vocabs
        else:
            data = self.prepare_dataset(condition_path, pt_file, src_data_iter, src_dir, src_path, tgt_data_iter,
                                        tgt_path, opt)
    ...
    def prepare_dataset(self, condition_path, pt_file, src_data_iter, src_dir, src_path, tgt_data_iter, tgt_path, opt):
        data = inputters. \
            build_dataset(self.fields,
                          self.data_type,
                          src_path=src_path,
        ...
        pos = 0
        while pos < len(data):
            print(f'processed {pos}')
            end = min(len(data), pos + size_once)
            exapmles = self.get_example(data, pos, end, opt)
            for i in range(pos, end):
                setattr(data.examples[i], 'graph', exapmles[i - pos].graph)
                if condition_path:
                    setattr(data.examples[i], 'condition_target', target_condition[i])

            pos += size_once

        data_examples = data.examples
        data.examples = [e for e in data_examples if getattr(e, 'graph') is not None]
        exlucde_lines = [i for i, e in enumerate(data_examples) if getattr(e, 'graph') is None]
        for i, e in enumerate(data.examples):
            e.indices = i
        data.src_vocabs = [v for e, v in zip(data_examples, data.src_vocabs) if getattr(e, 'graph') is not None]
        for i, e in enumerate(data.src_vocabs):
            e.indices = i

        # 去掉那些不能跃迁的分子,保存原来的文件
        self.save_good_example(src_path, set(exlucde_lines))
        self.save_good_example(condition_path, set(exlucde_lines))
        # We save fields in vocab.pt seperately, so make it empty.
        init_fields = data.fields
        data.fields = []
        with open(pt_file, 'wb') as f:
            pickle.dump(data, f)

        exlucde_lines_file = f"{os.path.dirname(pt_file)}/exlcude_index.pt"
        with open(exlucde_lines_file, 'wb') as f:
            pickle.dump(exlucde_lines, f)

        data.fields = init_fields
        return data

    def save_good_example(self, src_path, exclude_lines):
        main_name, ext = src_path.split('.')
        save_file = f"{main_name}_good.{ext}"
        with open(src_path) as f, open(save_file, 'w') as out:
            lines = f.readlines()
            out.writelines([line for i, line in enumerate(lines) if i not in exclude_lines])
        shutil.move(src_path, f"{main_name}_original.{ext}")
        shutil.move(save_file, src_path)

    import functools
    def get_example(self, data, pos, end, opt):
        if False:
            pool = multiprocessing.Pool()
            graphs = pool.map(partial(parrel_func, have_3d=opt.with_3d_confomer), data[pos:end])
            pool.close()
            pool.join()
            del pool
        else:
            graphs = [parrel_func(e, opt.with_3d_confomer) for e in data[pos:end]]
        return graphs
    ...
    def _fast_translate_batch(self,
        ...
        enc_states, memory_bank, src_lengths \
            = self.model.encoder((src, batch.graph, batch.condition_target), src_lengths)
        ...
        condition = make_condtion(batch.condition_target, max_length, memory_bank.device, memory_bank.dtype)
        if condition is not None:
            condition = tile(condition, beam_size, dim=0)
        for step in range(max_length):
            decoder_input = alive_seq[:, -1].view(1, -1, 1)
            cur_step_condition = None
            if condition is not None:
                cur_step_condition = condition[:, step:step + 1, :]
            # Decoder forward.
            dec_out, dec_states, attn = self.model.decoder(
                ...
                step=step, condition=cur_step_condition)

            if dec_out.shape[-1] < list(self.model.generator.parameters())[0].shape[-1]:
                # condition_of_every_atom = make_condtion(src[-1], decoder_outputs.size(0), decoder_outputs.device,
                #                                         decoder_outputs.dtype)
                emb_cond = cur_step_condition.transpose(0, 1).contiguous()
                dec_out = torch.cat((dec_out, emb_cond), dim=2)
            ...
            if is_finished.any():
                ...
                top_beam_finished |= is_finished[:, 0].eq(1).byte()
            ...
            if condition is not None:
                condition = condition.index_select(0, select_indices)
