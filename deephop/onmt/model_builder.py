...
def build_encoder(opt, embeddings):
    ...
    if opt.encoder_type == "transformer":
        return TransformerEncoder(opt.enc_layers, opt.enc_rnn_size,
                                  ...
                                  opt.dropout, embeddings, opt.condition_dim, opt.arch)
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    model = onmt.models.NMTModel(encoder, decoder, arch=model_opt.arch)
    ...
    if not model_opt.copy_attn:
        ...
        input_dim = model_opt.dec_rnn_size
        if model_opt.arch == 'after_decoding':
            input_dim = model_opt.dec_rnn_size + model_opt.condition_dim
        generator = nn.Sequential(
            nn.Linear(input_dim, len(fields["tgt"].vocab)),
            gen_func
        )
